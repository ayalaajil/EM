...
def moy_TildeData(Ftilde, X):
    ...
    for i in range(Ftilde.shape[2]):
        Ftilde_X[:, :, i] = np.matmul(Ftilde[:, :, i], X[i, :].T)
...
def moy_T_Prim_Data(YX, T):
    ...
    for i in range(YX.shape[0]):
        YX_Tprim[:, :, i] = np.matmul(YX[i, :].T, T[i, :])
...
def emsem_function(Y,X1,X2,T,T1,T2,epsilon=10**(-3),nb_it=100):
    ...
    # Get the regression coefficients of X2 on the first principal component of X2
    regX2_Fact_coefs = regX2_Fact.coef_

    # Create a matrix of initial A2 values using the regression coefficients
    ini_A2_Fact = np.tile(regX2_Fact_coefs, (qX2, 1))

    # Get the standard errors of the regression coefficients of X2 on the first principal component of X2
    regX2_Fact_se = np.sqrt(np.diag(np.linalg.inv(cl1X2_Fact.T @ cl1X2_Fact) @ ini_sigma2X2_Fact))

    # Set initial values of sigma2X2_Fact to the average of the squared standard errors
    ini_sigma2X2_Fact = np.mean(regX2_Fact_se ** 2)
    ...
    pca_Y = PCA(n_components=1, whiten=False)
    cl1Y_Fact = pca_Y.fit_transform(Y - T.dot(ini_D)).squeeze()   #donne la premiere composante d'apres FactomineR : correspond au facteur G

    # Fit a linear regression model of cl1Y_Fact on cl1X1_Fact and cl1X2_Fact
    reg_cl1Y = sm.OLS(cl1Y_Fact, sm.add_constant(np.column_stack((cl1X1_Fact, cl1X2_Fact)))).fit()

    # Divide cl1Y_Fact by the standard error of the regression residuals
    cl1Y_Fact /= reg_cl1Y.mse_resid ** 0.5

    # Fit a linear regression of Y on the first principal component of Y
    regY_Fact = LinearRegression().fit(cl1Y_Fact.reshape(-1, 1), (Y - np.dot(T, ini_D)))

    # Create a matrix of initial B values using the regression coefficients
    ini_B_Fact = np.tile(regY_Fact.coef_, (qY, 1))


    # Initialize an array of NA values with length qY
    sig_lm_qY_Fact = np.full(qY, np.nan)

    for i in range(qY):
        sig_lm_qY_Fact[i] = regY_Fact.summary()["sigma"][i]

    ini_sigma2Y_Fact = np.mean(sig_lm_qY_Fact ** 2) #initialisation du parametre sigma2Y 
    ...
    # D'o?
    Psi_X1 = np.diag(sigma2_X1_chap, qX1)  # necessary for loop to calculate M-i and Sigma_i of the distribution that gives F_tilde, G_tilde, Phi_tilde, and Gamma_tilde
    ...
    while diff > epsilon and it + 1 < nb_it:
        ...
        E1_1 = np.column_stack((((C1) ** 2 + (C2) ** 2 + 1) * B,
                              C1 * A1,
                              C2 * A2))  # c1 and c2 are real in the case of a factor, hence "*" and not "%*%"
        ...
        E2_1 = np.column_stack((((C1) ** 2 + (C2) ** 2 + 1) * B.T @ B + Psi_Y,
                              C1 * B.T @ A1,
                              C2 * B.T @ A2))  # c1 and c2 are real in the case of a factor, hence "*" and not "%*%"
        ...
        # New
        for i in range(n):
            E3[:, :, i] = np.concatenate(((Y[i, :] - D.T @ T[i, :]).T,
                                          (X1[i, :] - D1.T @ T1[i, :]).T,
                                          (X2[i, :] - D2.T @ T2[i, :]).T), axis=1)
        ...
        fg = np.prod(EE * E3, axis=0)  # fg is an array from i=1 to n with elements fg[,,i]
        ...
        G_tilde_Y_bar = np.mean(G_tilde * Y, axis=(0, 1))

        # Calculate G_tilde_T_prim_bar
        G_tilde_T_prim_bar = np.mean(G_tilde * T, axis=(0, 1))

        # Calculate G_tilde_T_bar
        G_tilde_T_bar = np.mean(G_tilde * T, axis=(0, 1))

        # Calculate F1_tilde_X1_bar
        F1_tilde_X1_bar = np.mean(F1_tilde * X1, axis=(0, 1))

        # Calculate F2_tilde_X2_bar
        F2_tilde_X2_bar = np.mean(F2_tilde * X2, axis=(0, 1))

        # Calculate F1_tilde_T1_prim_bar
        F1_tilde_T1_prim_bar = np.mean(F1_tilde * T1, axis=(0, 1))

        # Calculate F2_tilde_T2_prim_bar
        F2_tilde_T2_prim_bar = np.mean(F2_tilde * T2, axis=(0, 1))

        # Calculate F1_tilde_T1_bar
        F1_tilde_T1_bar = np.mean(F1_tilde * T1, axis=(0, 1))

        # Calculate F2_tilde_T2_bar
        F2_tilde_T2_bar = np.mean(F2_tilde * T2, axis=(0, 1))

        # Calculate Y_T_prim_bar
        Y_T_prim_bar = np.mean(Y * T, axis=(0, 1))

        # Calculate X1_T1_prim_bar
        X1_T1_prim_bar = np.mean(X1 * T1, axis=(0, 1))

        # Calculate X2_T2_prim_bar
        X2_T2_prim_bar = np.mean(X2 * T2, axis=(0, 1))
        ...
        F1G_tilde_bar = np.mean(F1_tilde * G_tilde, axis=(0, 1))

        # Calculate F2G_tilde_bar
        F2G_tilde_bar = np.mean(F2_tilde * G_tilde, axis=(0, 1))

        # Calculate F1F2_tilde_bar
        F1F2_tilde_bar = np.mean(F1_tilde * F2_tilde, axis=(0, 1))
        ...
        B = np.transpose((G_tilde_Y_bar - Y_T_prim_bar @ np.linalg.inv(T_T_prim_bar) @ G_tilde_T_bar) @ 
                    np.linalg.inv(Gamma_tilde_bar - G_tilde_T_prim_bar @ np.linalg.inv(T_T_prim_bar) @ G_tilde_T_bar))
